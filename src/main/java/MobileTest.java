...
public class MobileTest {
   ...
   public void checkMobilePayment() throws InterruptedException {
        ...
        Assert.assertEquals("79 UAH", driver.findElement(By.xpath("//div[@data-qa-node='amount']")).getText());

        Assert.assertEquals("2 UAH", driver.findElement(By.xpath("//div[@data-qa-node='commission']")).getText());

        Assert.assertEquals("Поповнення мобільного", driver.findElement(By.xpath("//div[@data-qa-node='category']")).getText());

        Assert.assertEquals("Поповнення телефону. На номер +380634477837", driver.findElement(By.xpath("//div[@data-qa-node='details']")).getText());
